...
static char stored_password[MAX_PASS_LEN] = {0};

EventGroupHandle_t wifi_event_group;
const int WIFI_CONNECTED_EVENT = BIT0;

static const char *EXAMPLE_PROV_SEC2_USERNAME = "wifiprov";
static const char *EXAMPLE_PROV_SEC2_PWD = "abcd1234";
...
static void wifi_prov_print_qr(const char *name, const char *username, const char *pop, const char *transport)
{
    ...
    if (pop)
    {
        snprintf(payload, sizeof(payload), "{\"ver\":\"%s\",\"name\":\"%s\""
                 ...
                 "v1", name, username, pop, transport);
    }
    else
    {
        snprintf(payload, sizeof(payload), "{\"ver\":\"%s\",\"name\":\"%s\""
...
void wifi_event_handler(void *arg, esp_event_base_t event_base, int32_t event_id, void *event_data)
{
    ...
    if (event_base == WIFI_PROV_EVENT)
    {
        ...
        case WIFI_PROV_CRED_FAIL:
        {
            ...
            retries++;
            if (retries >= 5) // Thay CONFIG_EXAMPLE_PROV_MGR_MAX_RETRY_CNT bằng số cụ thể nếu cần
            {
